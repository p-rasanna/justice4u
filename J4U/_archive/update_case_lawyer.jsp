...
<%
    ...
    if (caseIdStr == null || lawyerEmail == null || caseIdStr.isEmpty() || lawyerEmail.isEmpty()) {
        response.sendRedirect("clientdashboard.jsp?msg=Invalid Request Parameters");
        return;
    }
    try {
        ...
        psCust.close();
        if (customerId == -1) {
            response.sendRedirect("clientdashboard.jsp?msg=User Not Found");
            return;
        }
        // 2. Get Lawyer ID
        int lawyerId = -1;
        ...
        con.close();
        if (rows > 0) {
            response.sendRedirect("clientdashboard.jsp?msg=Request Sent to Lawyer");
        } else {
            // Debugging Output
            ...
            out.println("<a href='clientdashboard.jsp'>Back to Dashboard</a>");
            // response.sendRedirect("clientdashboard.jsp?msg=Update Failed - Case Not Found or Access Denied");
        }
    } catch (Exception e) {
        e.printStackTrace();
        response.sendRedirect("clientdashboard.jsp?msg=Error: " + e.getMessage());
    }
%>
