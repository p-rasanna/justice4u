...
public class LoginServlet extends HttpServlet {
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if ("lawyer".equals(role)) {
      loginPage = "auth/Lawyer_login_form.jsp";
    } else if ("client".equals(role)) {
      loginPage = "auth/cust_login.jsp";
    } else if ("intern".equals(role)) {
      loginPage = "auth/internlogin_form.jsp";
    }
    ...
    try {
      ...
      if ("admin".equals(role)) {
        ...
        query = "SELECT cid, cname, pass, COALESCE(verification_status, 'PENDING') as verification_status, COALESCE(profile_type, 'admin') as profile_type FROM cust_reg WHERE email = ?";
        redirectUrl = "client/clientdashboard.jsp"; // single unified dashboard
      }
      else if ("intern".equals(role)) {
      ...
      if (rs.next()) {
        ...
        if (PasswordUtil.verifyPassword(password, storedPass)) {
          if ("lawyer".equals(role) || "intern".equals(role)) {
            ...
            normalizedProfileType = cProfileType == null ? "admin" : cProfileType.trim().toLowerCase();
