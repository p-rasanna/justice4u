...
<%
    ...
    try(Connection con=DatabaseConfig.getConnection()){
        String sql="SELECT alid, cname, title, status, curdate FROM allotlawyer WHERE lname=?" + ("a".equals(f)?" AND status IN ('Accepted','IN_PROGRESS')":"") + " ORDER BY alid DESC";
...
<!DOCTYPE html>
<html lang="en">
<jsp:include page="components/_head.jsp"><jsp:param name="title" value="Case Repository"/></jsp:include>
                                    ...
                                    <div class="card border-0 shadow-sm rounded-4 h-100 attorney-card transition-base overflow-hidden">
                                        <div class="card-body p-4 d-flex flex-column">
                                            ...
                                            <div class="d-flex gap-2 mt-auto">
                                                <a href="viewcase.jsp?alid=<%= c[0] %>" class="btn btn-gold flex-fill rounded-3 py-2 fw-bold shadow-sm border-0">
                                                    Manage Case <i class="bi bi-gear-fill ms-2"></i>
                                                </a>
                                                <a href="client_messages.jsp?alid=<%= c[0] %>" class="btn btn-outline-dark rounded-3 px-3 shadow-sm position-relative">
                                                    <i class="bi bi-chat-left-dots-fill"></i>
                                                    <span class="position-absolute top-0 start-100 translate-middle p-1 bg-danger border border-light rounded-circle" style="display:none;"></span>
                                                </a>
                                            </div>
                                        </div>
                                    </div>
