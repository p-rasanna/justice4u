...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .text-muted-note { color: var(--ink-tertiary); font-size: 0.75rem; font-style: italic; }
    ...
    <div class="dashboard-shell">
        ...
        <div class="panel smart-enter d-2">
            <div class="panel-head">
                <h3><i class="ph ph-users panel-icon"></i> Pending & Verified Lawyers</h3>
            ...
            <div class="table-responsive">
              ...
              <c:choose>
                <c:when test="${not empty pendingLawyers}">
                  <c:forEach var="lawyer" items="${pendingLawyers}">
                    ...
                    <tr>
                      ...
                      <td><c:out value="${lawyer.amount}" /></td>
                      <td>
                        <span class="status-pill ${statusClass}">
                          <c:if test="${statusClass == 'status-pending'}"><i class='ph ph-clock'></i> </c:if>
                          <c:if test="${statusClass == 'status-verified'}"><i class='ph ph-check-circle'></i> </c:if>
                          <c:if test="${statusClass == 'status-rejected'}"><i class='ph ph-x-circle'></i> </c:if>
                      ...
                      <td>
                        ...
                        <c:choose>
                          <c:when test="${lawyer.docStatus == 'VERIFIED'}">
                            <a href="approvel.jsp?id=${lawyer.id}" class="btn-action btn-approve">
                              <i class="ph ph-check"></i> Approve
                            </a>
                          </c:when>
                          <c:otherwise>
                            <span class="text-muted-note"><i class="ph ph-warning-circle"></i> Verify docs first</span>
                          </c:otherwise>
                        </c:choose>
                        </div>
                      </td>
                ...
                <c:otherwise>
                  <tr>
                    <td colspan="14" class="text-center" style="padding: 24px; color: var(--ink-secondary);">No pending lawyer applications queue empty.</td>
                  </tr>
                </c:otherwise>
              </c:choose>
              </tbody>
